...
import com.pesna.items.Arrow;
import com.pesna.items.FrostArrow;
...
public class Player implements ScreenObject {
    ...
    public Player( Main _reference ) throws IOException {
        ...
        regenTimer = new SimpleTimer(4.0f);
        ShotTimer =new SimpleTimer(1f);
        ...
        //pesnaPlayer.speed = 1;
        SpriterAnimation.attack.animationSpeed = SpriterAnimation.attack.animationSpeed / 10;
    }
    ...
    public void draw( Main _reference )
    {
        ...
        if ( pesnaPlayer.flippedX() == 1 && flip)
            pesnaPlayer.flipX();
        if ( pesnaPlayer.flippedX() == -1 && !flip)
            pesnaPlayer.flipX();
    ...
    private boolean auxiliat = true;  private int shooting = 0;private boolean startShoot = true;
    int seted = 0;
    private void control() {
        ...
        if(shooting == 0)
        {
            ...
            if (Gdx.input.isKeyJustPressed(Keys.SPACE) && y == 0) {
                setAnimation(SpriterAnimation.jump);
                vspeed = +13;
            }
        ...
        if(Gdx.input.isButtonPressed(Input.Buttons.LEFT) || Gdx.input.isButtonPressed(Input.Buttons.RIGHT)) {
            if (ShotTimer.TimeElapsed()) startShoot = true;
            if(startShoot) {
                shooting = 1;
                if (seted == 0) {
                    setAnimation(SpriterAnimation.bowshoot);
                    seted = 1;

                }

                if (idlePlayer.getTime() >= 25) {
                    if(Gdx.input.isButtonPressed(Input.Buttons.RIGHT)) {
                        FrostArrow arrow = new FrostArrow(reference);
                        reference.screenManager.gameScreen.ObjectForceAdd(arrow);
                        startShoot = false;
                    }
                    if(Gdx.input.isButtonPressed(Input.Buttons.LEFT)) {
                        Arrow arrow = new Arrow(reference);
                        reference.screenManager.gameScreen.ObjectForceAdd(arrow);
                        startShoot = false;
                    }
                }
            }
            else setAnimation(SpriterAnimation.idle);
        }
        else {shooting = 0; seted = 0;}
